...
- The documentation can be automatically generated by going to the `<Agnes root>/doc/` folder, and executing the command `doxygen doxygen-config-file`. (This requires the Doxygen tool to be installed first.) This will create two subfolders `<Agnes root>/doc/html/` and `<Agnes root>/doc/latex/`, and the documentation can be opened in either of the following ways:
